################################################################
#### OpenPyStruct Single-Core Optimizer / Data Generator      ####
...
# Single-core execution (no parallel processing)
# num_workers = 22  # Removed as we're using single-core
...
# Fixed roller node locations
roller_nodes = [10, 30, 70, 85, num_nodes - 1]  # Assuming node numbering starts at 1
...
def generate_sample(sample_idx, num_nodes, flag, L, node_positions, roller_nodes, available_nodes, patience=10):
    ...
    # Progress bar for epochs within each sample
    for epoch in tqdm(range(max_e), desc=f"Sample {sample_idx+1}/{num_samples}", leave=False):
        optimizer.zero_grad()
        ops.wipe()  # Reset OpenSees model

        setup_model(I_tensor, node_positions, roller_nodes_sample, force_nodes, force_values, A, E, uniform_udl)

        ops.analysis('Static')
        try:
            ops.analyze(1)
        except Exception as e:
            print(f"OpenSees analysis failed for sample {sample_idx+1} at epoch {epoch+1}: {e}")
        ...
        try:
            ...
            print(f"Failed to retrieve responses for sample {sample_idx+1} at epoch {epoch+1}: {e}")
            break  # Skip to the next sample
        ...
        if patience_counter >= patience:
            #print(f"Stopping early at epoch {epoch} due to no improvement in {patience} epochs.")
            break
    ...
    # Return the results
    return {
        "roller_x_locations": [node_positions[node - 1] for node in roller_nodes_sample],
...
def main():
    ...
    with tqdm(total=num_samples, desc="Generating Samples") as pbar:
        for i in range(num_samples):
            result = generate_sample(i, num_nodes, flag, L, node_positions, roller_nodes, available_nodes, patience=patience)
