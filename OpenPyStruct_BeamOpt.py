...
roller_nodes = []
available_nodes = [
    n for n in range(2, num_nodes) if n != num_nodes
...
for _ in range(1, N_rollers):
    ...
    while not valid_choice:
        ...
        if all(
            abs(new_roller_node - existing_node) >= L_min
            for existing_node in roller_nodes
        ):
...
def setup_model(
    I_tensor, node_positions, roller_nodes, force_nodes, force_values, A, E, uniform_udl
):
    """
    Set up the OpenSees model using updated moments of inertia
    and apply uniform UDL to all elements.
    """
    ...
    for i in range(len(node_positions) - 1):
        ops.element(
            "elasticBeamColumn", i + 1, i + 1, i + 2, A, E, I_tensor[i].item(), 1
        )
...
def compute_combined_loss(I_tensor, E, G, alpha_moment, alpha_shear, node_positions):
    ...
    bending_moments = torch.tensor(
        bending_moments, dtype=torch.float32, requires_grad=True
    )
    shear_forces = torch.tensor(shear_forces, dtype=torch.float32, requires_grad=True)

    # Compute bending energy
    bending_energy = torch.sum(
        (bending_moments**2) / (2 * E * I_tensor + 1e-6)
...
loss_history = {"total": [], "primary": [], "bending_energy": [], "shear_energy": []}
...
for epoch in range(num_epochs):
    ...
    if no_improvement_epochs >= patience:
        msg = (
            ...
            f"No improvement in loss for {patience} epochs."
        )
        print(msg)
        break
    ...
    if (epoch + 1) % 100 == 0:
        print(
            ...
            f"Bending Energy: {bending_energy.item():.6f} | "
            f"Shear Energy: {shear_energy.item():.6f}"
        )
...
for i in range(len(I_values)):
    ...
    axs[0].plot(
        ...
        color="blue",
        alpha=0.3,
        label="Approx section height" if i == 0 else "",
    )

# Highlight Pin and Roller Supports
axs[0].scatter(
    0, 0, color="green", s=200, marker="^", label="Pin Support"
)  # Pin at x = 0
for node in roller_nodes:
    ...
    axs[0].scatter(
        ...
        s=200,
        marker="o",
        label="Roller Support" if node == roller_nodes[0] else "",
    )
if num_nodes not in roller_nodes:
    axs[0].scatter(
        L, 0, color="red", s=200, marker="o"
    )  # Explicitly add roller at x = L
...
for node, force in zip(force_nodes, force_values):
    ...
    axs[0].arrow(
        ...
        fc="red",
        ec="red",
        label="Point Load" if node == force_nodes[0] else "",
    )
...
axs[1].step(
    node_positions[:-1], shear_forces_kn, where="post", color="red", label="Shear Force"
)
