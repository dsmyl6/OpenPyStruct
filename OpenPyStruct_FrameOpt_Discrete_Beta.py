...
vertical_load = -1e4  # Uniform vertical load (N) applied to beam elements (negative for downward)
...
def setup_frame_model(I_tensor):
    ...
    The first num_columns entries of I_tensor are used for columns; the remainder for beams.
    ...
    for i in range(num_stories):
        for j in range(num_bays + 1):
            ...
            I_val = I_tensor[elem_tag - 1].item()  # columns occupy indices 0 .. num_columns-1
...
def compute_combined_loss(I_tensor):
    ...
    Run the analysis and compute the combined loss:
      total_loss = sum(I) + α_moment*(∑[bending_moment²/(2·E·I)]) + α_shear*(∑[shear_force²/(G·A_local)])
...
for epoch in range(num_epochs):
    ...
    if (epoch + 1) % 10 == 0:
        print(f"Epoch {epoch+1:3d}: Total Loss = {current_loss:.6e}, Primary = {primary_loss.item():.6e}")
...
for i in range(num_stories):
    for j in range(num_bays + 1):
        ...
        plt.plot([x_i, x_j], [y_i, y_j], 'b-', linewidth=lw,
...
for i in range(1, num_stories + 1):
    for j in range(num_bays):
        ...
        I_elem = opt_I[elem_index]
        lw = 15 * (I_elem / max_I_val)**(1/3)
        plt.plot([x_i, x_j], [y_i, y_j], 'r-', linewidth=lw,
